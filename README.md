...
```text
LiDAR + IMU + GNSS factor -> GLIM odometry fixed-lag smoother -> map
...
Loop closure가 필요한 route는 `configs/glim_gnss_odom_gpu_xy_loop`를 `config_path`로 넘깁니다. 단, 인천대교 같은 one-way 직선 주행은 loop closure 기회가 거의 없습니다.
...
| Test | Scans | Good `<2m,2deg` | XY p90 | Yaw p90 |
|---|---:|---:|---:|---:|
| 40-700s, exact init | 66 | 62.1% | 0.318 m | 0.282 deg |
| 40-700s, 2m/2deg perturb | 66 | 53.0% | 1.064 m | 1.376 deg |
| 40-700s, 5m/5deg perturb | 66 | 25.8% | 4.463 m | 3.976 deg |
| 108s short map, 2m/2deg perturb | 51 | 80.4% | 0.129 m | 0.267 deg |

Offline local VGICP odometry: `2170 frames`, `100% converged`, `mean 7.94 ms`, `p90 15.55 ms`.
