...
GlimROS::GlimROS(const rclcpp::NodeOptions& options) : Node("glim_ros", options) {
  ...
  min_raw_points_per_scan = config_ros.param<int>("glim_ros", "min_raw_points_per_scan", 0);
  min_points_time_delta = config_ros.param<double>("glim_ros", "min_points_time_delta", 0.0);
  last_accepted_points_stamp = -1.0;
...
size_t GlimROS::points_callback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr msg) {
  ...
  if (min_raw_points_per_scan > 0 && raw_points->size() < min_raw_points_per_scan) {
    spdlog::warn("skip a point cloud with too few raw points ({} < {})", raw_points->size(), min_raw_points_per_scan);
    return 0;
  }

  raw_points->stamp += points_time_offset;
  if (!time_keeper->process(raw_points)) {
    spdlog::warn("skip a point cloud with an invalid timestamp (stamp={:.6f})", raw_points->stamp);
    return 0;
  }

  if (min_points_time_delta > 0.0 && last_accepted_points_stamp > 0.0) {
    const double points_time_delta = raw_points->stamp - last_accepted_points_stamp;
    if (points_time_delta < min_points_time_delta) {
      spdlog::warn("skip a near-duplicate point cloud (stamp={:.6f} last={:.6f} diff={:.6f} < {:.6f})",
        raw_points->stamp,
        last_accepted_points_stamp,
        points_time_delta,
        min_points_time_delta);
      return 0;
    }
  }

  last_accepted_points_stamp = raw_points->stamp;
